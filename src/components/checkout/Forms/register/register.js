...
const CheckoutRegisterForm = ({nextForm,backForm,updateUserData}) => {
    ...
    const validationSchema = Yup.object().shape({
        ...
        cfactruc: Yup
            ...
            .when("cfact2", {
                is: true,
                then:(schema)=> Yup.string().min(11).max(11).required("Required")
            }),
    ...
    return (
        <form className={'inlineBlock'} onSubmit={handleSubmit(onSubmit)}>
            <Grid container fullWidth spacing={2}>
                ...
                {fieldBody.cfact2 === true &&
                    <Grid 
                        item 
                        size={{ xs: 12, sm: 12, md: 12, }}  
                        className={errors.cfactruc ? 'textField textFieldWhite textFieldError' :'textField textFieldWhite'}>
                        <TextField 
                            label="RUC." 
                            variant="filled" 
                            name="cfactruc"
                            ...
                            onChange={changeField}
                            {...register("cfactruc")}
                            error={errors.cfactruc ? true : false}
                        />
                    </Grid>
                }
                ...
                <Grid className={'textFielCheck'} item size={{ xs: 12, sm: 12, md: 12, }}>
                    <FormGroup>
                        <FormControlLabel 
                            label={<span>He leído y autorizo el tratamiento de mis datos según la <Link to={'/politicas-de-privacidad'}>Política de Privacidad</Link> y <Link to={'/terminos-y-condiciones'}> Términos y Condiciones</Link></span>  }
                            //required 
